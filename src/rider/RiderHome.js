...
export default class RiderHome extends React.Component {
	...
	componentDidMount() {
		// firebase
		// 	.database()
		// 	.ref('/Riders/' + firebase.auth().currentUser.uid + '/Details')
		// 	.once('value')
	...
	<Container>
		<Header style={{ height: 200, backgroundColor: '#42A5F5' }}>
			<Body style={{ alignItems: 'center', justifyContent: 'center' }}>
				...
				<Text style={{ color: '#ffffff', fontWeight: 'bold', fontSize: 20 }}>Username</Text>
			</Body>
		</Header>
		<Content>
			<DrawerItems {...props} />
...
const MyDrawerNav = createAppContainer(
	createDrawerNavigator(
		{
			Home: { screen: MyStackNav },
