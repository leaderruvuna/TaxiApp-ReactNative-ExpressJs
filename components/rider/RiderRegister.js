...
import { StyleSheet, Text, View ,Button,YellowBox,AsyncStorage,TouchableOpacity,TextInput,KeyboardAvoidingView,Image,Platform,ActivityIndicator} from 'react-native';
...
export default class RiderRegister extends React.Component {
 ...
 constructor(props){
      ...
      <View style={styles.container}>
        ...
        */}
        ...
        <TouchableOpacity
          ...
          >
          <Image source={require('../Images/googleplus.png')} style={{width:20,height:20}}  />
        </TouchableOpacity>
        <TouchableOpacity
          style={styles.facebookButton}
          onPress={()=>alert('this is facebook')}
          >
        <Image source={require('../Images/facebook.png')} style={{width:20,height:20}}  />
        </TouchableOpacity>

        </View>
        <View style={styles.RideNames}>
        <TextInput style={styles.textInputfn}
        ...
        <TextInput style={styles.textInputln}
  ...
  _VerifyAsync = async () => {
    ...
    if(this.state.firstname.trim() === "" || this.state.email.trim() === "" || this.state.mobile.trim() === "" || this.state.lastname.trim() === "" || this.state.password.length=='')
    {
      Toast.show("All inputs must be filled!",Toast.SHORT,Toast.TOP,ToastStyle);
      return;
    }
  ...
  if (firebase.auth().currentUser) {
    ...
    if (userId) {
      ...
      firebase.database().ref(`Riders/${userId}/Details`).set({
...
const sendSMSVerification = (res, uid, phone) => {
                ...
                .then(message => {
                  firebase.database().ref(`/users/${uid}/verification`)
                  .set(verification)
                  .then(() => {
